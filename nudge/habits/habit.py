...
class Habit:
    ...
    def current_streak(self) -> int:
        ...
        match self.periodicity:
            case self.periodicity.DAILY:
                streak = self._current_streak_daily(today)
            case self.periodicity.WEEKLY:
                streak = self._current_streak_weekly(today)
            case self.periodicity.WEEKLY_FIXED_DAY:
                streak = self._current_streak_weekly_fixed_day(today)
            case self.periodicity.MONTHLY:
                streak = self._current_streak_monthly(today)
            case self.periodicity.MONTHLY_FIXED_DAY:
                streak = self._current_streak_monthly_fixed_day(today)

        return streak

    def _current_streak_daily(self, today: datetime) -> int:
        streak = 0
        timestamps = sorted(self.completion_timestamps, reverse=True)
        # Streak for daily: consecutive days
        if (today - self.completion_timestamps[0].date()).days <= 1:
            streak = 1
        for i in range(1, len(self.completion_timestamps)):
            current_date = self.completion_timestamps[i].date()
    ...
    def _current_streak_weekly(self, today: datetime) -> int:
        streak = 0
        timestamps = sorted(self.completion_timestamps, reverse=True)
    ...
    def _current_streak_monthly(self, today: datetime) -> int:
        streak = 0
        timestamps = sorted(self.completion_timestamps, reverse=True)

        # Streak for flexible monthly: completed in consecutive months
        if (today.year - timestamps[0].date().year) * 12 + (
            today.month - timestamps[0].date().month
        ) <= 1:
            streak = 1
    ...
    def _current_streak_weekly_fixed_day(self, today: datetime) -> int:
        streak = 0
        timestamps = sorted(self.completion_timestamps, reverse=True)
    ...
    def _current_streak_monthly_fixed_day(self, today: datetime) -> int:
        streak = 0
        timestamps = sorted(self.completion_timestamps, reverse=True)
        ...
        if (
            today_day == recent_day
            and (today.year - recent_date.year) * 12 + (today.month - recent_date.month) <= 1
        ):
            streak = 1
        ...
        for i in range(1, len(timestamps)):
            ...
            if (
                ...
                and (previous_month - current_month) == 1
            ):
                streak += 1
            else:
                break
